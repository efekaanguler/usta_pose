#!/usr/bin/env python3
"""
Synchronized 4-Camera Session Recorder (FFV1 RGB Intermediate Edition)
...
Color is stored during recording as lossless FFV1 in color_lossless.mkv.
After recording stops, each color_lossless.mkv is transcoded to color.mp4
with libx264 slow preset for the downstream pipeline.
Depth is stored as **lossless FFV1 video** in MKV container (16-bit grayscale).

Output structure:
    recordings/session_YYYYMMDD_HHMMSS/
        cam1/color_lossless.mkv  cam1/color.mp4  cam1/depth.mkv
        cam2/color_lossless.mkv  cam2/color.mp4  cam2/depth.mkv
        cam3/color_lossless.mkv  cam3/color.mp4  cam3/depth.mkv
        cam4/color_lossless.mkv  cam4/color.mp4  cam4/depth.mkv
        metadata.json
...
Usage:
    python felfelfeci3_1.py --output-dir ./recordings --offline-crf 20
...
class CameraThread:
    ...
    def prepare_recording(self, session_dir):
        ...
        # Color intermediate writer: lossless FFV1, transcoded after recording.
        video_path = os.path.join(self.cam_dir, "color_lossless.mkv")
...
def main(args):
    ...
    print(f"  Color realtime format: FFV1 lossless MKV (color_lossless.mkv)")
        ...
        for i, cam in enumerate(cameras):
            ...
            metadata['cameras'][str(i + 1)] = {
                ...
                'color_storage': {
                    ...
                    'container': 'mp4',
                    'generated_from': f"cam{i + 1}/color_lossless.mkv",
                    'transcode_status': color_info.get('status'),
                },
                'color_intermediate_storage': {
                    'format': 'ffv1_mkv',
                    'file': f"cam{i + 1}/color_lossless.mkv",
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description=(
            ...
            "(FFV1 lossless color intermediate -> offline libx264 color.mp4, FFV1 depth)"
        ),
    ...
    parser.add_argument('--delete-intermediate', action='store_true',
                        help='Delete color_lossless.mkv after a successful color.mp4 transcode')
    parser.add_argument(
        '--align-depth-live',
        action='store_true',
        help=(
