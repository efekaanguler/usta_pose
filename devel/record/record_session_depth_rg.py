#!/usr/bin/env python3
"""
Synchronized 4-Camera Session Recorder (Depth as RG-encoded MP4)

Records color video + depth video from 4 RealSense cameras, where the
16-bit depth (z16) is split across two 8-bit channels:
    R channel = high byte  (depth >> 8)
    G channel = low byte   (depth & 0xFF)
    B channel = 0

To reconstruct depth:
    depth_uint16 = R.astype(np.uint16) * 256 + G.astype(np.uint16)
    depth_meters = depth_uint16.astype(np.float32) * depth_scale
...
NOTE: MP4 video compression (even with high quality settings) is lossy,
so there will be minor quantization artifacts in the recovered depth.
For truly lossless depth, use felfeci_record_session.py (HDF5).
...
Usage:
    python record_session_depth_rg.py --output-dir ./recordings
...
def encode_depth_to_rg(depth_z16):
    """Encode a 16-bit depth frame into R (high byte) and G (low byte) of a BGR image.

    Args:
        depth_z16: (H, W) uint16 array of raw z16 depth values

    Returns:
        (H, W, 3) uint8 BGR image where:
            B channel = 0
            G channel = low 8 bits of depth  (depth & 0xFF)
            R channel = high 8 bits of depth (depth >> 8)

    To reconstruct:
        depth = R.astype(np.uint16) * 256 + G.astype(np.uint16)
    """
    depth = depth_z16.astype(np.uint16)
    high = (depth >> 8).astype(np.uint8)   # R channel
    low = (depth & 0xFF).astype(np.uint8)  # G channel

    # BGR format: B=0, G=low, R=high
...
def decode_rg_to_depth(bgr_image):
    """Decode an RG-encoded BGR image back to a 16-bit depth frame.

    Args:
        bgr_image: (H, W, 3) uint8 BGR image from the depth.mp4 video

    Returns:
        (H, W) uint16 depth frame
    """
    high = bgr_image[:, :, 2].astype(np.uint16)  # R channel
    low = bgr_image[:, :, 1].astype(np.uint16)   # G channel
    return high * 256 + low
...
class CameraThread:
    ...
    def __init__(
        ...
        self.depth_video_writer = None  # writer for RG-encoded depth.mp4
        self.cam_dir = None
    ...
    def _writer_loop(self):
        """Disk I/O thread — writes color video and RG-encoded depth video.

        Depth frames are encoded by splitting the 16-bit z16 value:
            R channel = high byte (depth >> 8)
            G channel = low byte  (depth & 0xFF)
            B channel = 0
        """
        while True:
            ...
            if depth_image is not None and self.depth_video_writer is not None:
                rg_encoded = encode_depth_to_rg(depth_image)
                self.depth_video_writer.write(rg_encoded)
    ...
    def prepare_recording(self, session_dir):
        ...
        # Depth video writer — RG-encoded, same resolution and FPS as color
        if self.enable_depth:
...
def main(args):
    ...
    print(f"  Depth output: RG-encoded MP4 (R=high byte, G=low byte)")
        ...
        for i, cam in enumerate(cameras):
            metadata['cameras'][str(i + 1)] = {
                ...
                'depth_storage': {
                    ...
                    'channel_mapping': {
                        'R': 'high_byte (depth >> 8)',
                        'G': 'low_byte (depth & 0xFF)',
                        'B': 'unused (0)',
                    },
                    'reconstruction': (
                        'depth_uint16 = R.astype(uint16) * 256 + G.astype(uint16); '
                        'depth_meters = depth_uint16.astype(float32) * depth_scale'
                    ),
                    'note': (
                        'Depth is stored as RG-encoded MP4. The 16-bit z16 depth '
                        'value is split into high byte (R channel) and low byte '
                        '(G channel). MP4 compression introduces minor lossy '
                        'artifacts. For truly lossless depth use '
                        'felfeci_record_session.py (HDF5).'
                    ),
                },
            }
    ...
    window_name = "4-Camera Session (R=Record, Q=Quit) [Depth→RG MP4]"
    if use_gui:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description="Record synchronized color + depth (RG-encoded MP4) from 4 RealSense cameras",
