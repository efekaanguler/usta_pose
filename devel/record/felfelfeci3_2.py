#!/usr/bin/env python3
"""
Synchronized 4-Camera Session Recorder (Ultrafast H.264 Intermediate Edition)
...
Color is stored during recording as realtime libx264 ultrafast in
color_realtime.mp4. After recording stops, each color_realtime.mp4 is
transcoded to color.mp4 with libx264 slow preset for the downstream pipeline.
Depth is stored as **lossless FFV1 video** in MKV container (16-bit grayscale).

Output structure:
    recordings/session_YYYYMMDD_HHMMSS/
        cam1/color_realtime.mp4  cam1/color.mp4  cam1/depth.mkv
        cam2/color_realtime.mp4  cam2/color.mp4  cam2/depth.mkv
        cam3/color_realtime.mp4  cam3/color.mp4  cam3/depth.mkv
        cam4/color_realtime.mp4  cam4/color.mp4  cam4/depth.mkv
        metadata.json
...
Usage:
    python felfelfeci3_2.py --output-dir ./recordings --realtime-crf 18 --offline-crf 20

    Serial numbers are read from camera_config.json (see --cam-config).

Requirements:
    ffmpeg CLI available on PATH
"""
...
class CameraThread:
    ...
    def __init__(
        ...
        self.realtime_crf = realtime_crf
    ...
    def prepare_recording(self, session_dir):
        ...
        # Color intermediate writer: realtime H.264, transcoded after recording.
        video_path = os.path.join(self.cam_dir, "color_realtime.mp4")
...
def transcode_color_intermediates(session_dir, num_cameras, intermediate_name, args):
    """Convert per-camera color intermediates to final color.mp4 files."""
    ffmpeg = get_ffmpeg_executable()
    results = {}

    for cam_id in range(1, num_cameras + 1):
        rel_input = f"cam{cam_id}/{intermediate_name}"
        rel_output = f"cam{cam_id}/color.mp4"
        input_path = os.path.join(session_dir, rel_input)
        output_path = os.path.join(session_dir, rel_output)
        result = {
            'input_file': rel_input,
            'output_file': rel_output,
            'codec': 'libx264',
            'preset': args.offline_preset,
            'crf': args.offline_crf,
        ...
        if not os.path.exists(input_path):
            ...
            results[str(cam_id)] = result
            print(f"[Color Transcode] Missing {input_path}")
            continue
        ...
        print(
            f"[Color Transcode] cam{cam_id}: {intermediate_name} -> color.mp4 "
            f"(libx264 {args.offline_preset}, CRF {args.offline_crf})"
        )
        ...
        try:
            ...
            if ok:
                ...
                if args.delete_intermediate:
                    ...
                    result['intermediate_deleted'] = True
                    print(f"[Color Transcode] cam{cam_id}: deleted {rel_input}")
            else:
                result['stderr_tail'] = completed.stderr[-2000:]
                print(f"[Color Transcode] cam{cam_id}: FAILED")
        except Exception as exc:
            result['seconds'] = round(time.perf_counter() - started, 3)
            result['status'] = 'failed'
            result['error'] = repr(exc)
            print(f"[Color Transcode] cam{cam_id}: FAILED ({exc})")

        results[str(cam_id)] = result

    ok_count = sum(1 for item in results.values() if item.get('status') == 'ok')
    print(f"[Color Transcode] Completed {ok_count}/{num_cameras} camera files.")
    return results
...
def main(args):
    ...
    print(f"  Color realtime format: libx264 MP4 (CRF {args.realtime_crf}, preset ultrafast)")
    print(f"  Color offline transcode: libx264 MP4 (CRF {args.offline_crf}, preset {args.offline_preset})")
    print(f"  Delete color intermediate: {'YES' if args.delete_intermediate else 'NO'}")
    ...
    for i in range(num_cameras):
        ...
        cam = CameraThread(
            ...
            realtime_crf=args.realtime_crf,
        )
        cameras.append(cam)
    ...
    def stop_recording_session():
        ...
        color_transcode = transcode_color_intermediates(
            session_dir,
            num_cameras,
            "color_realtime.mp4",
            args,
        ...
        metadata = {
            ...
            'color_transcode_completed': datetime.now().isoformat(),
            'resolution': {'width': args.width, 'height': args.height},
            'fps': args.fps,
            'color_transcode': color_transcode,
        ...
        for i, cam in enumerate(cameras):
            color_info = color_transcode.get(str(i + 1), {})
            metadata['cameras'][str(i + 1)] = {
                ...
                'color_storage': {
                    'format': 'mp4',
                    'file': f"cam{i + 1}/color.mp4",
                    'codec': 'libx264',
                    'crf': args.offline_crf,
                    'preset': args.offline_preset,
                    'pixel_format_in': 'auto',
                    'pixel_format_out': 'yuv420p',
                    'container': 'mp4',
                    'generated_from': f"cam{i + 1}/color_realtime.mp4",
                    'transcode_status': color_info.get('status'),
                },
                'color_intermediate_storage': {
                    'format': 'mp4',
                    'file': f"cam{i + 1}/color_realtime.mp4",
                    'codec': 'libx264',
                    'crf': args.realtime_crf,
                    'preset': 'ultrafast',
                    'tune': 'zerolatency',
                    ...
                    'container': 'mp4',
                    'lossless': False,
                    'deleted_after_transcode': color_info.get('intermediate_deleted', False),
                },
                'depth_storage': {
                    'format': 'ffv1_mkv',
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description=(
            ...
            "(realtime libx264 ultrafast color intermediate -> offline libx264 color.mp4, FFV1 depth)"
        ),
    ...
    parser.add_argument('--realtime-crf', type=int, default=18,
                        help='CRF for realtime libx264 ultrafast color_realtime.mp4')
    parser.add_argument('--offline-crf', type=int, default=20,
                        help='CRF for final offline libx264 color.mp4 transcode')
    parser.add_argument('--offline-preset', type=str, default='slow',
                        choices=[
                            'ultrafast', 'superfast', 'veryfast', 'faster',
                            'fast', 'medium', 'slow', 'slower', 'veryslow',
                        ],
                        help='Preset for final offline libx264 color.mp4 transcode')
    parser.add_argument('--delete-intermediate', action='store_true',
                        help='Delete color_realtime.mp4 after a successful color.mp4 transcode')
    parser.add_argument(
        '--align-depth-live',
        action='store_true',
        help=(
