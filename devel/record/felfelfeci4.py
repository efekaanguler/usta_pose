#!/usr/bin/env python3
"""
Synchronized 4-Camera Session Recorder (Full FFV1 Lossless MKV Edition — v4)

Changes from felfelfeci3:
  - Same capture logic; benefits from host-level tuning applied before launch:
    CPU governor=performance, xhci IRQ spread to all cores, USB autosuspend
    disabled for all RealSense devices.
...
class CameraThread:
    ...
    def __init__(
        ...
        self.height = height
        self.fps = fps
        # Depth can run at a lower resolution than color to reduce USB/CPU load.
        # D455 struggles to deliver color+depth both at 1280x720@30fps on a
        # shared USB controller; 848x480 depth keeps the stereo module within
        # its bandwidth budget while color stays full-res.
        self.depth_width = depth_width if depth_width else self.width
        self.depth_height = depth_height if depth_height else self.height
    ...
    def start(self):
        ...
        if self.enable_depth:
            cfg.enable_stream(rs.stream.depth, self.depth_width, self.depth_height, rs.format.z16, self.fps)
        ...
        for sensor in device.sensors:
            # Disable auto-exposure priority so the sensor never sacrifices FPS
            # to achieve a longer exposure in low light.
...
def main(args):
    ...
    # Depth is enabled per camera: pose cameras always record depth; gaze cameras
    # only record depth when --gaze-depth is passed.  Disabling depth on the two
    # D435i gaze cameras reduces USB load enough for all four cameras to sustain
    # 30 fps without duplicate frames on a shared USB 3.2 controller.
    depth_enabled = [
        True if roles[i] == 'pose' else args.gaze_depth
        for i in range(num_cameras)
    ]
    for i in range(num_cameras):
        depth_tag = 'color+depth' if depth_enabled[i] else 'color only'
        print(f"  Cam {i + 1} ({roles[i]}, {depth_tag}): {serials[i] or 'NOT FOUND'}")
    ...
    print(f"  Depth format: FFV1 lossless MKV (16-bit grayscale, pose cameras only by default)")
    print("  Exposure: AUTO (auto_exposure_priority=0, FPS-priority mode)")
    dw = args.depth_width or args.width
    dh = args.depth_height or args.height
    if dw != args.width or dh != args.height:
        print(f"  Depth resolution: {dw}x{dh} (color: {args.width}x{args.height})")
    else:
        print(f"  Depth resolution: {dw}x{dh} (same as color)")
        ...
        for i, cam in enumerate(cameras):
            metadata['cameras'][str(i + 1)] = {
                ...
                'depth_storage': (
                    ...
                    } if cam.enable_depth else {'enabled': False}
                ),
            }
...
if __name__ == '__main__':
    ...
    parser.add_argument('--width', type=int, default=1280, help='Color frame width')
    parser.add_argument('--height', type=int, default=720, help='Color frame height')
    parser.add_argument('--fps', type=int, default=30, help='Frames per second')
    parser.add_argument('--depth-width', type=int, default=0,
                        help='Depth stream width (default: same as --width)')
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            'Enable depth recording for gaze cameras (cam3, cam4). '
            'Disabled by default because running depth on all four cameras '
            'saturates the shared USB controller and causes duplicate frames. '
            'Enable only if gaze depth data is needed and USB load permits.'
        ),
    )
    parser.add_argument(
        '--align-depth-live',
