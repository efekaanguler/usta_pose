#!/usr/bin/env python3
"""
Synchronized 4-Camera Session Recorder (Luxury H.264 + FFV1 Depth Edition)
...
Color is stored with imageio-ffmpeg/libx264 CRF 15 and slow preset.
...
Usage:
    python felfelfeci2.py --output-dir ./recordings
...
class CameraThread:
    ...
    def _writer_loop(self):
        ...
        Runs in a separate thread so that FFmpeg writes and imageio appends
    ...
    def prepare_recording(self, session_dir):
        ...
        # Color video writer: H.264 CRF 15, slow preset, near visually lossless.
        video_path = os.path.join(self.cam_dir, "color.mp4")
        self._color_writer = iio_ff.write_frames(
            video_path,
            ...
            fps=self.fps,
            macro_block_size=1,
            output_params=['-crf', '15', '-preset', 'slow', '-movflags', '+faststart'],
        )
        self._color_writer.send(None)
...
def main(args):
    ...
    print(f"  Color format: libx264 MP4 (CRF 15, preset slow)")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description="Record synchronized color + depth from 4 RealSense cameras (libx264 color + FFV1 depth)",
